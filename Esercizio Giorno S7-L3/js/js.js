...
function updateCart() {
  ...
  cartArr.forEach((x) => {
    ...
    btnRem.addEventListener("click", function () {
      listItem.remove();
      let index = cartArr.findIndex((cartItem) => cartItem.title === x.title);
      if (index !== -1) {
        cartArr.splice(index, 1);
      }
      localStorage.setItem("itemCart", JSON.stringify(cartArr));
      updateCart();
    });
...
function createList(books) {
  ...
  books.forEach((x) => {
    ...
    btnAdd.addEventListener("click", function createCart() {
      let cart = document.querySelector("#cart");
      let listItem = document.createElement("li");
      listItem.classList.add("m-2","border","border-secondary","rounded","p-2","d-flex","align-items-center","justify-content-between"
      );
